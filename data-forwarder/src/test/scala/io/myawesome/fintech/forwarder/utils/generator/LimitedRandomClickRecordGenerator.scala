...
class LimitedRandomClickRecordGenerator[F[_]: Monad](using random: Random[F])
    extends ClickRecordGenerator[F] {
  ...
  private val campaign =
    List("Black Friday", "Christmas", "Unidays", "campaign A", "campaign B")
  private val channel =
    List("Email", "Social Media", "Search Engine", "Direct", "Affiliate")
  private val referrers =
    List("google.com", "facebook.com", "twitter.com", "linkedin.com", "none")

  private def randomOption[T](
    generator: F[T],
    chance:    Double = 0.5,
  ): F[Option[T]] =
    random.nextDouble
      .map(_ < chance)
      .ifM(generator.map(Some(_)), none[T].pure[F])
  ...
  override def generateOne: F[ClickRecord] =
    for {
      sessionId <- random.nextIntBounded(uuids.length).map(uuids)
      browser <- randomOption(
        random.nextIntBounded(browsers.length).map(browsers),
      )
      campaign <- randomOption(
        random.nextIntBounded(campaign.length).map(campaign),
      )
      channel <- random.nextIntBounded(channel.length).map(channel)
      referrer <- randomOption(
        random.nextIntBounded(referrers.length).map(referrers),
      )
