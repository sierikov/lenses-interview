...
object Main extends IOApp.Simple {
  ...
  import io.myawesome.fintech.forwarder.Codecs.*
  ...
  given Resource[IO, KeySerializer[IO, String]] =
    avroSerializer[String].forKey(avroSettings)
  ...
  private def stream(
      generator: RandomClickRecordGenerator[IO]
  ): Stream[IO, Unit] = {
    KafkaProducer
      ...
      .handleErrorWith { case ex: Exception =>
        Stream.emit(
          logger.error(s"Failed to send record! Error: ${ex.getMessage}")
        )
      }
  }
}
